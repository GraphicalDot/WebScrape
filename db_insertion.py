...
class DBInsert(object):
	...
	def db_insert_eateries(eatery):
		...
		try:
                        # flag to indicate if index needs to be created
                        index_flag = False

                        if not ZomatoEateries.count():
                                # if there are no docs in the collection
                                # set the index flag to True which will 
                                # create the index after first doc insertion
                                index_flag = True
			...
			ZomatoEateries.update({"eatery_id": eatery.get("eatery_id")}, {"$set": eatery}, upsert=True)
                        ...
                        try:
                                ...
                                # if index_flag is true, create the index entry
                                if index_flag:
                                        ZomatoEateries.create_index([("eatery_id", pymongo.ASCENDING)], unique=True)
	...
	def db_insert_reviews(reviews):
		...
		for review in reviews:
			try:
                ...
                if index_flag:
                        ZomatoReviews.create_index([("review_id", pymongo.ASCENDING)], unique=True)
	...
	def db_insert_users(reviews):
		...
		for review in reviews:
			try:
                ...
                if index_flag:
                        ZomatoUsers.create_index([("user_id", pymongo.ASCENDING)], unique=True)
