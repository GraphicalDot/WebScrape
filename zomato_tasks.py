...
@app.task(bind=True, max_retries=3, retry=True, acks_late=True, default_retry_delay=5*60)
def get_eateries_list(self, url, number_of_resturants, skip, is_eatery):
    try:
        print terminal.blue(" >> get_eateries_list")
        obj = EateriesList(url, number_of_resturants, skip, is_eatery)
        result = obj.eateries_list
        return result
    except Exception as exc:
        print terminal.red('Task Failed : task id {0.id}, args: {0.args!r} kwargs: {0.kwargs!r}'.format(self.request))
        # retrying the task
        self.retry(exc=exc)


@app.task(bind=True, max_retries=3, acks_late=True, default_retry_delay=5*60)
def execute_all_eateries(self, eateries_list, callback_func):
    try:
        print terminal.blue('>> execute_all_eateries')
        callback = subtask(callback_func)
        return group(callback.clone([arg,]) for arg in eateries_list)()
    except Exception as exc:
        print terminal.red('Task Failed : task id {0.id}, args: {0.args!r} kwargs: {0.kwargs!r}'.format(self.request))
...
@app.task(bind=True, ignore_result=True, max_retries=5, acks_late=True, default_retry_delay=5*100)
def get_eatery_info(self, eatery_dict, flush_eatery=False):
    # setting the flag to lock the task with unique task id
    have_lock = False

    # creating unique task id
    url_digest = md5(eatery_dict['eatery_url']).hexdigest()
    lock_id = '{0}-lock-{1}'.format(self.name, url_digest)

    # creating the lock using Redis with the unique task id
    the_lock = redis.Redis().lock(lock_id)

    try:
        # return

        # Note:
        #   the locking is performed here because of the issue of selenium getting restarted in
        #   another process with same params without terminating the already running one;
        #   this creates two or more processes running selenium driver with same eatery_urls.
        #   So to avoid it, whenever a new task starts, a unique id is created using the 
        #   eatery_url, and the task is locked until it fails or completes.

        # have_lock will become True if the lock is acquired successfully
        have_lock = the_lock.acquire(blocking=False)

        if not have_lock:
            # if the lock is not acquired then simply return
            # because another task is running with same parameters
            # and has not yet released lock
            print terminal.on_red("LOCK not ACQUIRED")
            return

        print terminal.on_blue("Task LOCKED!")
        print terminal.on_blue("lock_id : "+str(lock_id))

        print terminal.blue('>> get_eatery_info')
        if flush_eatery:
            DBInsert.flush_eatery(eatery_dict)
        ...
        # release the lock before returning back
        if have_lock:
            the_lock.release()
            print terminal.on_red("LOCK Released!")

        return
    except Exception as exc:
        print terminal.red('Task Failed : task id {0.id}, args: {0.args!r} kwargs: {0.kwargs!r}'.format(self.request))
        print terminal.red("Failed Task : "+str(self.request.id)+"\nError : "+str(exc))
        # release the lock in case its acquired and the task fails
        if have_lock:
            the_lock.release()
            print terminal.on_red("LOCK Released!")
        # terminating the task on exception
        revoke(self.request.id, terminate=True)


@app.task(bind=True, ignore_result=True, max_retries=3, acks_late=True, default_retry_delay=5)
def start_scraping(self, url, number_of_restaurants, skip, is_eatery):
    try:
        print terminal.blue(" >> start_scraping")
        process_list = get_eateries_list.s(url, number_of_restaurants, skip, is_eatery) | execute_all_eateries.s(get_eatery_info.s())
        process_list.delay()
    except Exception as exc:
        print terminal.red('Task Failed : task id {0.id}, args: {0.args!r} kwargs: {0.kwargs!r}'.format(self.request))
