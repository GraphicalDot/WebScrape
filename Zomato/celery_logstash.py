#Initial code copied from https://github.com/CyberInt/celstash/blob/master/celstash.py
...
class TaskCtxAdapter(logging.LoggerAdapter):
        ...
        def process(self, msg, kwargs):
                ...
                kwargs = kwargs.copy()
                kwargs.setdefault('extra', {})['celery'] = vars(celery.current_task.request)
                return msg, kwargs
...
def new_logger(name):
        ...
        handler = logstash.LogstashHandler(config.logstash.host, config.logstash.port)
        log.addHandler(handler)
        
        create_logdir(config.logdir)
        handler = TimedRotatingFileHandler('%s.json' % join(config.logdir, name), when='midnight', utc=True,)
