...
api_key = "AIzaSyAjA3iklf1CxtPUZHfoj_ey6QdTH382gx4"
...
pictures_database = config.get("zomato", "picsdatabase")
pictures_collection = config.get("zomato", "picscollection")
zomato_database = config.get("zomato", "database")
users_reviews_collection = config.get("zomato", "reviews")
eateries = config.get("zomato", "eatery")
...
# from connections import pictures_collection, users_reviews_collection, eateries
...
def find_google_places(eatery_id, __eatery_id, eatery_name, location):
        ...
        try:
                ...
                connection.zomato_database.eateries.update({"eatery_id": eatery_id}, {"$set": {"google": google}}, upsert=False)
        ...
        connection.zomato.eateries.update({"eatery_id": eatery_id}, {"$set": {"google": google}}, upsert=False)
        return google


def google_already_present(eatery_id, google):
            if google == "No entry exists on google":
                    connection.zomato.eateries.update({"eatery_id": eatery_id}, {"$set": {"google": "No entry exists on google"}}, upsert=False)
                    return 
            ...
            eatery_result = connection.zomato.eateries.find_one({"eatery_id": eatery_id})
...
def insert_images(__eatery_id, eatery_name, pic_list):
        for pic in pic_list:
                ...
                try:
                        print pictures_database.pictures_collection.insert(pic)
                except Exception as e:
                        print terminal.red(str(e))
...
def insert_reviews(eatery_id, __eatery_id, eatery_name, review_list):
        ...
        for review in review_list:
            if review.get("text"):
                ...
                connection.review.update({"fb_id": fb_id, "eatery_name": eatery_name, "name": name, "epoch": epoch, "review_text": review_text, "review_id": review_id, "eatery_id": eatery_id, "__eatery_id": __eatery_id})
                print review
                try:
                        print terminal.green(str(connection.zomato_database.users_reviews_collection.insert(review)))
                except Exception as e:
                        print terminal.red(str(e))
...
if __name__ == "__main__" :
  location = ["28.5887184513","77.1690523624"]
  x = find_google_places('3466', 'adafd32edw', "QD's Restaurant", location)
  pprint.pprint(x)
'''