...
# import pprint
# chrome_driver_exec_path = "driver/chromedriver"
...
class ZomatoReviews(object):
    ...
    def __get_all_reviews(self):
        ...
        for item in self.reviews_list:
            ...
            __reviews['user_rating'] = self.__get_user_rating(item)
    ...
    @exception_handling
    def __get_user_rating(self, review):
        if self.review_desc:
            rated = self.review_desc.find("div")['aria-label']
            rated = rated.replace('Rated', '')
            return eval(rated)
...
def make_soup(url):
    # driver = webdriver.Chrome(chrome_driver_exec_path)
...
if __name__ == "__main__":
    ...
    pprint.pprint(obj.reviews_data[8])
'''